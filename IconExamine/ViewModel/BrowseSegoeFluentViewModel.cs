...
public partial class BrowseSegoeFluentViewModel : ObservableObject
{
    const string Filename = "SegoeFluentDefinitionsProject.json";
    [ObservableProperty]
    ...
    public BrowseSegoeFluentViewModel()
    {
        ...
        GlyphIds = new ObservableCollection<LightGlyphDefn>(from i in Singleton<SupportedTables>.Instance.SegoeUIFluent select new LightGlyphDefn() { GlyphId = i, UnicodeChar = char.ConvertFromUtf32(i) });
    ...
    partial void OnActiveDefinitionChanged(SymbolIconDefinitions? oldValue, SymbolIconDefinitions newValue)
    {
        ...
        if(newValue.UsedFor!= GlyphUsedFor.SegoeFluent)
        {
